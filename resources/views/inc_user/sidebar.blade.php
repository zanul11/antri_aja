...
<div class="sidebar-wrapper sidebar-theme">
    ...
    <nav id="sidebar">
        ...
        <ul class="list-unstyled menu-categories" id="accordionExample">
            ...
            $hasCaret = (!empty($menu['caret'])) ? '<div> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-chevron-right">
                    <polyline points="9 18 15 12 9 6"></polyline>
                </svg></div>' : '';
            $hasIcon = (!empty($menu['icon'])) ? '<i data-feather="'. $menu['icon'] .'"></i>' : '';
            $hasTitle = (!empty($menu['title'])) ? '<span>'. $menu['title'] .'</span>' : '';
            $hasActive = ($category_name === $menu['title'] ) ? 'active' : '';
            $dataActive = ($category_name === $menu['title']) ? 'true' : 'false';
            $hasSub = (!empty($menu['sub_menu'])) ? 'collapse' : '';
            @endphp

            <li class="menu {{$hasActive}}">
                <a href="{{$menu['url']}}" data-active="{{ ($category_name === $menu['title']) ? 'true' : 'false' }}" data-toggle="{{$hasSub}}" aria-expanded="{{ ($category_name === $menu['title']) ? 'true' : 'false' }}" class="dropdown-toggle">
                    <div class="">
                        @php
                        echo $menu['icon'];
                        @endphp
                ...
                @foreach ($menu['sub_menu'] as $key => $child)
                @php
                $checkShow = ($category_name === $menu['title'] ) ? 'show' : '';
                $childActive = ($page_name === $child['title']) ? 'active' : '';
                @endphp
                <ul class="collapse submenu list-unstyled  {{$checkShow}}" id="{{$menu['title']}}" data-parent="#accordionExample">
